...
export default function EditParticipantModal({ competitionId, majorType, participant, onClose, onSuccess }: Props) {
  ...
  const { data, error: fetchError } = useSWR(
    `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/tournament-field?tournament=${majorType}`,
    fetcher,
  ...
  // Initialize selected players from the participant's current selections
  React.useEffect(() => {
    const initialPlayers = participant.player_selections.map(selection => {
      const player = availablePlayers.find((p: Player) => p.name === selection.player_name);
      return player || { name: selection.player_name, world_ranking: 0 };
    });
    setSelectedPlayers(initialPlayers);
  }, [participant.player_selections, availablePlayers]);
